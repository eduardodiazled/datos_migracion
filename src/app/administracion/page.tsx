...
export default function Dashboard() {
    ...
    const [providerRenewals, setProviderRenewals] = useState<any[]>([])
    const [hiddenRenewals, setHiddenRenewals] = useState<number[]>([])
    const [isLoaded, setIsLoaded] = useState(false)
    const [expandedGroups, setExpandedGroups] = useState<Record<string, boolean>>({
        'AYER': true, 'HOY': true, 'MAÑANA': true, 'PROXIMOS': true
    })

    // Load hidden state from local storage on mount (Hydration Safe)
    useEffect(() => {
        const savedHidden = localStorage.getItem('hidden_provider_renewals')
        if (savedHidden) {
            setHiddenRenewals(JSON.parse(savedHidden))
        }
        setIsLoaded(true)
    }, [])

    // Save hidden state to local storage when it changes
    useEffect(() => {
        if (isLoaded) {
            localStorage.setItem('hidden_provider_renewals', JSON.stringify(hiddenRenewals))
        }
    }, [hiddenRenewals, isLoaded])
    ...
    const handlePayPayroll = async () => {
        const amountStr = prompt(`Confirmar Pago de Nómina\n\nMonto acumulado: $${payroll.accumulated.toLocaleString()}\n\nIngrese el monto a pagar:`, payroll.accumulated.toString())
        if (!amountStr) return
        ...
        const expenseRes = await createExpense({
            category: 'NOMINA',
            description: `Pago de Nómina Acumulada (${payroll.days} días)`,
            amount: amount,
            paymentMethod: 'NEQUI', // O podrías preguntar
            date: new Date().toISOString(),
            supplier: 'Equipo'
        })

        if (!expenseRes.success) {
            return toast.error('Error creando el gasto: ' + expenseRes.error, { id: toastId })
        }

        // 2. Reset Counter
        const res = await resetPayroll()
        if (res.success) {
            toast.success('Pago registrado y nómina reiniciada', { id: toastId })
            loadDashboard()
        } else {
            toast.error('Gasto creado pero error reiniciando contador: ' + res.error, { id: toastId })
        }
    }
    ...
    async function loadDashboard() {
        setLoading(true)
        const [data, payrollData, renewalsData] = await Promise.all([
            getDashboardStats(selectedYear, selectedMonth),
            getPayrollStatus(),
            getUpcomingProviderPayments()
        ])
        setStats(data)
        setPayroll(payrollData)
        if (renewalsData.success) {
            setProviderRenewals(renewalsData.reminders || [])
        }
        setLoading(false)
    }

    const handlePayProvider = async (renewal: any) => {
        const amountStr = prompt(`Confirmar Pago a Proveedor\n\nServicio: ${renewal.serviceName}\nProveedor: ${renewal.providerName}\n\nIngrese el monto pagado:`, '15000')
        if (!amountStr) return
        ...
        try {
            const res = await createExpense({
                ...
                description: `Pago Renovación ${renewal.serviceName} - ${renewal.providerName}`,
                amount: amount,
                paymentMethod: 'NEQUI', // Default or ask?
                date: new Date().toISOString(), // Pay Today
                supplier: renewal.providerName
            })

            if (res.success) {
                toast.success('Pago registrado correctamente', { id: toastId })
                // Hide locally
                setHiddenRenewals(prev => [...prev, renewal.accountId])
                loadDashboard() // Refresh stats
            } else {
                toast.error('Error al registrar: ' + res.error, { id: toastId })
            }
        } catch (e) {
    ...
    return (
        <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 pb-24">
            ...
            {/* --- PROVIDER RENEWALS (AUTOPAYS) --- */}
            {providerRenewals.filter(r => !hiddenRenewals.includes(r.accountId)).length > 0 && (
                <div className="bg-slate-900/50 border border-violet-500/20 rounded-3xl p-6 relative overflow-hidden animate-in fade-in slide-in-from-top-4">
                    ...
                    <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
                        ...
                        Agenda de Pagos a Proveedores
                    </h2>

                    <div className="space-y-4">
                        {['AYER', 'HOY', 'MAÑANA', 'PROXIMOS'].map(groupKey => {
                            let groupLabel = groupKey
                            let groupColor = "text-white"
                            let items = []

                            if (groupKey === 'AYER') {
                                items = providerRenewals.filter(r => r.daysLeft === -1 && !hiddenRenewals.includes(r.accountId))
                                groupLabel = "Vencidos (Ayer)"
                                groupColor = "text-rose-500"
                            } else if (groupKey === 'HOY') {
                                items = providerRenewals.filter(r => r.daysLeft === 0 && !hiddenRenewals.includes(r.accountId))
                                groupLabel = "Para Hoy"
                                groupColor = "text-rose-400"
                            } else if (groupKey === 'MAÑANA') {
                                items = providerRenewals.filter(r => r.daysLeft === 1 && !hiddenRenewals.includes(r.accountId))
                                groupLabel = "Para Mañana"
                                groupColor = "text-amber-400"
                            } else {
                                // 2 or 3 days
                                items = providerRenewals.filter(r => r.daysLeft >= 2 && !hiddenRenewals.includes(r.accountId))
                                groupLabel = "Próximos Días"
                            ...
                            return (
                                <div key={groupKey} className="bg-slate-950/50 border border-white/5 rounded-2xl overflow-hidden">
                                    <button
                                        onClick={() => setExpandedGroups(prev => ({ ...prev, [groupKey]: !prev[groupKey] }))}
                                        className="w-full flex items-center justify-between p-4 hover:bg-white/5 transition"
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={`font-black text-sm uppercase tracking-wider ${groupColor}`}>{groupLabel}</div>
                                            <span className="bg-slate-800 text-xs font-bold px-2 py-0.5 rounded-full text-slate-400">{items.length} topers</span>
                                        </div>
                                        <div className={`transition-transform duration-300 text-slate-500 ${isExpanded ? 'rotate-180' : ''}`}>▼</div>
                                    </button>

                                    {isExpanded && (
                                        <div className="p-4 pt-0 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 animate-in fade-in slide-in-from-top-1">
                                            {items.map((renewal, idx) => {
                                                const isUrgent = renewal.daysLeft <= 0
                                                const isWarning = renewal.daysLeft === 1
                                                ...
                                                if (isUrgent) {
                                                    ...
                                                    statusClass = "bg-amber-500 text-black font-bold"
                                                } else {
                                                ...
                                                return (
                                                    <div key={idx} className={`p-4 rounded-xl border flex flex-col justify-between gap-3 transition hover:scale-[1.02] ${bgClass}`}>
                                                        <div>
                                                            <div className="flex justify-between items-start mb-2">
                                                                <span className={`text-[10px] uppercase font-bold px-2 py-1 rounded-full ${statusClass}`}>
                                                                    {renewal.status}
                                                                </span>
                                                                <button
                                                                    onClick={(e) => {
                                                            ...
                                                            <div className="font-bold text-white text-base leading-tight mb-1 truncate" title={renewal.providerName}>{renewal.providerName}</div>
                                                            <div className="text-xs text-slate-400 font-medium truncate">{renewal.serviceName}</div>
                                                            {renewal.email && <div className="text-[10px] text-slate-500 truncate mt-0.5">{renewal.email}</div>}
                                                        </div>

                                                        <button
                                                            onClick={(e) => {
                                                                e.stopPropagation()
                                                                handlePayProvider(renewal)
                                                            }}
                                                            className="w-full py-2 rounded-lg bg-white/5 hover:bg-white/10 text-white text-[10px] font-bold border border-white/10 flex items-center justify-center gap-2 transition"
                                                        >
                                                            <DollarSign size={12} className="text-emerald-400" /> Registrar Pago
                                                        </button>
                                                    </div>
                                                )
                                            })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                {/* Promotions / Rotations Card */}
                {Object.keys(stats.promotions || {}).length > 0 && (
                    <div className="bg-gradient-to-br from-amber-500/20 to-orange-600/20 border border-amber-500/30 p-6 rounded-3xl relative overflow-hidden group hover:border-amber-500/50 transition-all md:col-span-1">
                        <div className="flex items-center gap-3 mb-4">
                            <div className="w-10 h-10 rounded-full bg-amber-500/20 flex items-center justify-center text-amber-400">
                                <Zap size={20} />
                            </div>
                            <span className="text-amber-200 font-bold text-sm uppercase tracking-wider">Para Promocionar</span>
                        </div>
                        <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar pr-2">
                            {Object.entries(stats.promotions).map(([key, count]) => (
                                <div key={key} className="flex justify-between items-center bg-black/20 p-2 rounded-lg border border-white/5">
                                    <span className="text-xs font-bold text-white truncate max-w-[70%]">{key}</span>
                                    <span className="text-xs font-black text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full">{count as number} unds</span>
                                </div>
                            ))}
                        </div>
                    </div>
                )}
                {/* Restock / Zero Stock Card */}
                {stats.restock && stats.restock.length > 0 && (
                    <div className="bg-gradient-to-br from-red-500/20 to-pink-600/20 border border-red-500/30 p-6 rounded-3xl relative overflow-hidden group hover:border-red-500/50 transition-all md:col-span-1">
                        <div className="flex items-center gap-3 mb-4">
                            <div className="w-10 h-10 rounded-full bg-red-500/20 flex items-center justify-center text-red-400">
                                <ShoppingCart size={20} />
                            </div>
                            <span className="text-red-200 font-bold text-sm uppercase tracking-wider">COMPRA ESTOS (0)</span>
                        </div>
                        <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar pr-2">
                            {stats.restock.map((service: string) => (
                                <div key={service} className="flex justify-between items-center bg-black/20 p-2 rounded-lg border border-white/5">
                                    <span className="text-xs font-bold text-white truncate w-full">{service}</span>
                                </div>
                            ))}
                        </div>
                    </div>
